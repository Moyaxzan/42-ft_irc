...
bool Command::invite(Client *client, Server *server, const std::string& line) {
	...
	if (chan->isMember(target)) // is invited client already on channel ?
		return (client->bufferMessage(server, ERR_USERONCHANNEL(invitorNick, targetNick, channel)), false);
	chan->addInvited(target);
	target->bufferMessage(server, RPL_INVITING(invitorNick, invitorUsername, channel, targetNick));
	client->bufferMessage(server, INVITECONFIRMED(invitorNick, channel, targetNick));
	return (true);
}
